...
HAC.simboot.newton <- function(model = c("GAM", "SCAM", "Krig"), k = 10) {
  ...
  #}
    ...
    cat("\n Monotonically increasing smooth (mpi) \n")
		...
		boot.fun <- function(data, i) {
		  ...
		  Y0 <- R * Hstar + sample(data$res, size = 1, replace = TRUE)
          if (all(i == 1:n)) {
              inv.predict.newton(HAC.mpi, y = R*Hstar, x.name = "specs", start = N, interval = FALSE)
          } else {
              inv.predict.newton(boot.fit, y = Y0, x.name = "specs", start = N, interval = FALSE)
          }
		...
		boot.fun <- function(data, i) {
		  ...
		  Y0 <- R * Hstar + sample(data$res, size = 1, replace = TRUE)
          if (all(i == 1:n)) {
              inv.predict.newton(HAC.cv, y = R*Hstar, x.name = "specs", start = N, interval = FALSE)
          } else {
              inv.predict.newton(boot.fit, y = Y0, x.name = "specs", start = N, interval = FALSE)
          }
		...
		boot.fun <- function(data, i) {
		  ...
		  Y0 <- R * Hstar + sample(data$res, size = 1, replace = TRUE)
          if (all(i == 1:n)) {
              inv.predict.newton(HAC.micv, y = R*Hstar, x.name = "specs", start = N, interval = FALSE)
          } else {
              inv.predict.newton(boot.fit, y = Y0, x.name = "specs", start = N, interval = FALSE)
          }
