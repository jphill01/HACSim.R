...
HAC.simpost <- function(model = c("GAM", "SCAM", "Krig"), k = 10){
	...
	n <- 10000
	g <- data.frame(specs = seq(1, ceiling(Nstar - X), length = 10000)) # fine grid for predictor
	...
	if (model == "GAM") {
        ...
        bias <- mu - ceiling(Nstar - X) # bias
        cat("Bias: ", bias, "\n")
        
        rel.bias <- bias / mu # relative bias
        cat("Rel. bias: ", rel.bias, "\n")
